    ...
    async with async_playwright() as p:
        try:
            ...
            for vacancy in vacancies:
                ...
                existing = session.query(Vacancy).filter(Vacancy.url == vacancy["link"]).first()
                if existing:
                    skipped += 1
                    continue
            ...
            logger.info(f"Собрано вакансий: {len(vacancies)}")
            logger.info(f"Пропущено дубликатов: {skipped}")
            
            session.commit()
            
            logger.info(f"Сохранено в базу: {len(vacancies)-skipped}")
